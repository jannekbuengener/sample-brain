...
| **sqlite-vec backend** | ✅ Opt-in on `main` | `SqliteVecSearchBackend` + vec0 rebuild via `index_build --search-backend sqlite-vec`. Availability: `vec status` / `vec smoke`. |
| **Search backend adapter** | ✅ Done | `get_search_backend("numpy" \| "sqlite-vec")` — same `SearchHit` format; NumPy is reference for correctness gates. |
...
| **FAISS index** | ❌ Superseded (ADR-0004) | ADR-0002 documents historical FAISS design. Index strategy on `main` is NumPy default + sqlite-vec opt-in; FAISS not implemented. |
...
4. A vector index can be built from stored embeddings via `index_build` — NumPy `.npz` (default) or sqlite-vec `vec0` cache (opt-in; [ADR-0004](adr/ADR-0004-sqlite-vec-search-backend.md))
...
7. **No vector index as source of truth.** NumPy `.npz`, sqlite-vec `vec0`, or historical FAISS caches can be deleted and rebuilt at any time without data loss.
