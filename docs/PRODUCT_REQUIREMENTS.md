...
**Local Workbench MVP** is a tkinter-based local purpose UI — not the VST product target. It exposes scan/analyze/classify logic on a chosen folder without FL Studio export, semantic search, or cloud sync. **Shipped:** waveform as play surface; cue/loop/attack metadata edit modes; attack suggestion (`Attack vorschlagen` / `Vorschlag übernehmen`); loop once-preview; context-sensitive usage hints; double-click row and Space for preview. **Planned next:** endless loop playback (see [`WORKBENCH_CUE_METADATA_PLAN.md`](WORKBENCH_CUE_METADATA_PLAN.md)). Start: `python -m src.cli workbench` (or `sample-brain workbench`). GUI smoke: [`WORKBENCH_GUI_SMOKE.md`](WORKBENCH_GUI_SMOKE.md).
