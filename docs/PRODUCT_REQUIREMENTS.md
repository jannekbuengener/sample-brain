...
Local Workbench (MVP): folder → analyze in-process → playlist + detail. Shipped follow-ups (#117, PRs #119–#140+): cancel, path entry, filter, sort, detail path polish, last-folder memory, CSV export, library cache v1, library folder list, audio preview, read-only waveform envelope, **cue metadata v1** (local `workbench_library.db`, read-only marker), preview from saved cue, **waveform play controls** (waveform is play surface; no visible Play/Stop buttons), **Shift+click permanent cue set**, **loop region display + loop edit mode** (toggle, two-click set, clear). Attack editing: follow-up — [`WORKBENCH_CUE_METADATA_PLAN.md`](WORKBENCH_CUE_METADATA_PLAN.md). Original sample files never modified by workbench.
```

**Local Workbench MVP** is a tkinter-based local purpose UI — not the VST product target. It exposes scan/analyze/classify logic on a chosen folder without FL Studio export, semantic search, or cloud sync. **Shipped:** waveform as play surface (left-click from saved cue, right-click from click position, Shift+left-click permanent cue set); double-click row and Space for preview; read-only waveform + saved cue marker + loop region display/edit (toggle mode, two-click set, clear). **Planned next:** attack marker/editor ([`WORKBENCH_CUE_METADATA_PLAN.md`](WORKBENCH_CUE_METADATA_PLAN.md)). Start: `python -m src.cli workbench` (or `sample-brain workbench`).
