...
**Local Workbench MVP** is a tkinter-based local purpose UI — not the VST product target. It exposes scan/analyze/classify logic on a chosen folder without FL Studio export, semantic search, or cloud sync. **Shipped:** audio preview from saved cue (`cue_start_ms`), read-only waveform + click-to-set cue. **Planned next:** loop region UI ([`WORKBENCH_CUE_METADATA_PLAN.md`](WORKBENCH_CUE_METADATA_PLAN.md)). Start: `python -m src.cli workbench` (or `sample-brain workbench`).
