...
### 4.9 Vector Index Builder (`src/index.py`, `src/vec_index.py`, EPIC 2)

- **Responsibility:** Read embeddings from SQLite, build NumPy cosine index, persist `.npz` archive and metadata, or rebuild sqlite-vec vec0 cache
- **Must not do:** Generate embeddings, own SQLite schema
- **Inputs:** `sample_embeddings` table
- **Outputs:** NumPy `.npz` at `data/indexes/` or custom `--index-path`; vec0 cache inside SQLite DB for sqlite-vec backend
- **Status:** NumPy default on `main`; sqlite-vec opt-in via `--search-backend sqlite-vec`
...
```
 ...
 │     embeddings (sqlite-vec opt-in; NumPy default)    │
 └──────────────────────────┬───────────────────────────┘
                            │
                            ▼
 ┌──────────────────────────────────────────────────────┐
...
**Key design property:** SQLite is the source of truth for all metadata. Generated artifacts (NumPy indexes, sqlite-vec caches, tag files, reports, caches) are always rebuildable from the SQLite catalog.
...
| FAISS index files | `data/indexes/*.faiss` | N/A | Superseded by ADR-0004 (sqlite-vec) |
...
**Decision:** Embedding vectors are stored as BLOBs in SQLite. NumPy `.npz` is the default index format. sqlite-vec `vec0` is the opt-in performance cache (ADR-0004).
...
- 512-dim vectors compatible with NumPy/sqlite-vec indexes
...
### 7.3 FAISS — Superseded (ADR-0002, Historical Record)

**Status on `main`:** Not imported. Not implemented. Superseded by [ADR-0004](adr/ADR-0004-sqlite-vec-search-backend.md) (sqlite-vec). NumPy `.npz` remains default search backend; sqlite-vec `vec0` is the opt-in performance cache.

**Historical context (ADR-0002, kept unchanged):**
- FAISS (`faiss-cpu`) was the original candidate for local vector index
- ADR-0002 documented FAISS as planned; ADR-0004 superseded it with sqlite-vec
- FAISS was never implemented on `main`; the ADR-0002 file remains as historical record

**Index lifecycle (as originally envisioned):**
...
**Current state:** See [ADR-0004](adr/ADR-0004-sqlite-vec-search-backend.md) for the active search backend strategy.
...
6. **All state is rebuildable where possible.** The SQLite catalog is the source of truth. Vector indexes (NumPy `.npz`, sqlite-vec `vec0`), reports, and tag exports are caches that can be regenerated.
