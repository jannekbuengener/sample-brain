...
| `rights` | `ownership_authorized`, `third_party_elements_cleared_for_resale`, `cleared_for_sampling` | each is `{ "status": "ok|unknown", "value": true|false|null, "source_ref": string }`; `ownership_authorized` and `third_party_elements_cleared_for_resale` require `true` for readiness (`false` blocks as `HOLD`); `cleared_for_sampling` permits `true` or `false`; `null`/`unknown` is unresolved (`HOLD`) |

`false` is an explicit decision, not an unknown value. For `ownership_authorized`
and `third_party_elements_cleared_for_resale`, `false` is a resolved negative
rights assertion that blocks readiness (`HOLD`). For `cleared_for_sampling`,
`true` and `false` are both valid resolved licensing choices. `null` or `unknown`
remains an unresolved value (`HOLD`). This contract gives no legal advice and
does not determine whether any decision is correct.
...
semantic evidence, missing composer, denied rights assertions (where
`ownership_authorized` or `third_party_elements_cleared_for_resale` is `false`),
unresolved rights assertions (`null`/`unknown`), unresolved sampling choice,
invalid listing data, and unknown CSV support. It must not conflate
`POND5_READY` with CSV completeness or with a legal conclusion.
