...
- **Partial:** Workbench MVP: waveform play surface (#117) — left-click plays from saved cue; right-click temp play at click; Shift+left-click sets cue permanently; read-only loop region when loop metadata set; loop edit planned ([`WORKBENCH_LOOP_EDIT_PLAN.md`](WORKBENCH_LOOP_EDIT_PLAN.md)); no loop/attack edit yet.
