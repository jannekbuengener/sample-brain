...
- **Branch:** `main` @ `62fea8b` (workbench detail path polish #122)
- **Open PRs:** 1 Dependabot — **#103** (numpy 2.5.1) → **HOLD_DEPENDENCY_VALIDATION** (`numba` requires `numpy<2.5`)
- **Open issues:** 3 — [#117](https://github.com/jannekbuengener/sample-brain/issues/117) Workbench usability follow-ups (P0 complete; P1+ open), [#73](https://github.com/jannekbuengener/sample-brain/issues/73) CLAP Tier-B, [#74](https://github.com/jannekbuengener/sample-brain/issues/74) sqlite-vec ANN
...
- **Tests:** 277 passed, 1 skipped (`pytest -q`); optional `[clap]` tests skip on HOLD vocal spike margins
...
- **Local Workbench** — tkinter MVP: enter or pick folder path, cancel mid-analysis, filter/sort playlist, segment-wise detail paths + copy, run in-process analyze + rule-based type (`workbench` subcommand)
...
- **Workbench audio preview / catalog DB / search integration** — MVP is folder-only; no playback, no last-folder memory, no catalog DB bind, no semantic search UI
